...
var messageReceived = make(chan mqtt.Message)
...
func main() {
	...
	for messagesLeft && !shouldExit {
		...
		for shouldHalt {
			...
			fmt.Println("waiting for terminal command.........")

			// Block the execution until an MQTT message is received
			<-messageReceived
	...
	token := client.Publish("playback_end", 0, false, "playback is done.....")
	token.Wait()
	...
	if topic == "mqtt-replay-record/replay" {
		if shouldHalt {
			...
			if payload == PLAY {
				...
				// Send signal on the messageReceived channel
				shouldHalt = true
			}
		}
