...
var message_handler mqtt.MessageHandler = func(client mqtt.Client, msg mqtt.Message) {
	...
	if topic == "mqtt-replay-record/record" {
		fmt.Println("received the mqtt-replay-record......")
		log.Printf("received the mqtt-replay-record..........")
		shouldExit = true
...
var publish_recording_topic string
var filenameTimestamp string
var record_done string
...
func main() {
	...
	if token := client.Subscribe("mqtt-replay-record/record", 0, nil); token.Wait() && token.Error() != nil {
		log.Fatalln("Error subscribing to mqtt-replay-record/record:", token.Error())
	}
	...
	for {
		...
		// exit request to stop recording
		if shouldExit {
			log.Printf("exit request here ")
			publish_recording_topic = "recording"

			token := client.Publish(publish_recording_topic, 0, false, "end")
			token.Wait()
